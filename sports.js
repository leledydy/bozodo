// sports.js — Fresh (≤2 days) Europe/Asia stories with validated images
// Returns article title/url/image + summary to keep posts fact-aligned.
...
// ---------- Fetch pipeline (Guardian first, then RSS) ----------
const parser = new RSSParser({
  customFields: {
    item: [
      ["media:content", "media", { keepArray: true }],
      ["media:thumbnail", "mthumb", { keepArray: true }]
    ]
  }
});
...
async function guardianSearch({ sport, fromISO, toISO }) {
  ...
  const qMap = {
    soccer:"football", boxing:"boxing", hockey:"ice hockey OR hockey", golf:"golf",
    basketball:"basketball", volleyball:"volleyball", "table tennis":"\"table tennis\"",
    badminton:"badminton", mma:"mma OR ufc", tennis:"tennis", "formula 1":"\"formula 1\" OR f1",
    cycling:"cycling"
  };
...
async function fetchOgDescription(url) {
  try {
    ...
    const desc = og || (html.match(/<meta name="description" content="([^"]+)"/i) || [])[1];
...
export function buildCaptionAndHashtags(sport, story) {
  ...
  const base = (generateHashtags(sport) || "").split(/\s+/);
...
export async function getSportPostData(sportArg) {
  ...
  const filtered = candidates.filter(c => {
    ...
    return t.isValid() && t.isAfter(fromDay) && t.isBefore(now.add(1, "day"));
  });
  ...
  for (const c of filtered) {
    ...
    if (img) {
      ...
      const ogDesc = await fetchOgDescription(c.url);
      const summaryText = c.summary || ogDesc || "";
      ...
      return {
        ...
        caption,
        hashtags,
        summaryText,   // <- used to anchor the LLM output
        factsText: summaryText
      };
    }
  }

  // If none with validated image: still skip (keeps your feed clean)
